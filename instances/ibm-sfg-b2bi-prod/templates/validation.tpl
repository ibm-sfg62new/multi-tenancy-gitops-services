...
{{- if eq $isServiceEnabled "true" -}}
    ...
    {{- if .Values.documentService.readBufferSize -}}
        {{- $result := include "integerValidation" .Values.documentService.readBufferSize -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.maxTotalConnections -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.maxTotalConnections -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.maxConnectionsPerRoute -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.maxConnectionsPerRoute -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.connectTimeout -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.connectTimeout -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.readTimeout -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.readTimeout -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.idleTimeout -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.idleTimeout -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.waitTimeout -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.waitTimeout -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.keepAlive -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.keepAlive -}}
        {{- if eq $result "false" -}}
	...
	{{- if .Values.documentService.connectionPoolConfig.retryCount -}}
        {{- $result := include "integerValidation" .Values.documentService.connectionPoolConfig.retryCount -}}
        {{- if eq $result "false" -}}
...
{{- if not ( or (eq $defaultDocStorageType "DB") (eq $defaultDocStorageType "FS") (eq $defaultDocStorageType "OS") ) -}}
{{- fail "Provide a valid value for .Values.setupCfg.defaultDocumentStorageType from one of these options - DB | FS | OS" -}}
{{- end -}}
{{- end -}}
...
{{- $isEnabled := .Values.integrations.itxIntegration.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxIntegration.enabled. Value can be either false or true." -}}
{{- end -}}

{{- if (eq $isEnabled "true") -}}

{{- $isEnabled := .Values.integrations.itxIntegration.dataSetup.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxIntegration.dataSetup.enabled. Value can be either false or true." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxIntegration.image.repository -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxIntegration.image.repository cannot be empty." -}}
{{- end -}}

{{- if not .Values.integrations.itxIntegration.image.digest -}}
{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxIntegration.image.tag -}}
{{- if eq $result "false" -}}
{{- fail "Please specify either .Values.integrations.itxIntegration.image.tag or .Values.integrations.itxIntegration.image.digest " -}}
{{- end -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxIntegration.image.pullPolicy -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxIntegration.image.pullPolicy cannot be empty." -}}
{{- else -}}
{{- $result := .Values.integrations.itxIntegration.image.pullPolicy -}}
{{- if not (or (eq $result "IfNotPresent") (eq $result "Always") (eq $result "Never")) -}}
{{- fail ".Values.integrations.itxIntegration.image.pullPolicy is not valid. Valid values are IfNotPresent , Always or Never" -}}
{{- end -}}
{{- end -}}

{{- $isEnabled := .Values.integrations.itxIntegration.log.includeHostInLogNames | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for .Values.integrations.itxIntegration.log.includeHostInLogNames. Value can be either false or true." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxIntegration.log.jniLog.level -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxIntegration.log.jniLog.level cannot be empty." -}}
{{- else -}}
{{- $result := .Values.integrations.itxIntegration.log.jniLog.level -}}
{{- if not (or (eq $result "none") (eq $result "failures") (eq $result "info") (eq $result "all")) -}}
{{- fail ".Values.integrations.itxIntegration.log.jniLog.level is not valid. Valid values are 'none', 'failures', 'info' or 'all'" -}}
{{- end -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxIntegration.log.cmgrLog.level -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxIntegration.log.cmgrLog.level cannot be empty." -}}
{{- else -}}
{{- $result := .Values.integrations.itxIntegration.log.cmgrLog.level -}}
{{- if not (or (eq $result "none") (eq $result "failures") (eq $result "info") (eq $result "all")) -}}
{{- fail ".Values.integrations.itxIntegration.log.cmgrLog.level is not valid. Valid values are 'none', 'failures', 'info' or 'all'" -}}
{{- end -}}
{{- end -}}

{{- $isEnabled := .Values.integrations.itxIntegration.dataPVC.useDynamicProvisioning | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for .Values.integrations.itxIntegration.dataPVC.useDynamicProvisioning. Value can be either false or true." -}}
{{- end -}}

{{- $isValid := .Values.integrations.itxIntegration.dataPVC.accessMode -}}
{{- if not ( or (eq $isValid "ReadWriteOnce") (eq $isValid "ReadOnlyMany") (eq $isValid "ReadWriteMany")) -}}
{{- fail "Please specify .Values.integrations.itxIntegration.dataPVC.accessMode as one of these supported values - ReadWriteOnce | ReadOnlyMany | ReadWriteMany" -}}
{{- end -}}

{{- $isEnabled := .Values.integrations.itxIntegration.logsPVC.useDynamicProvisioning | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for .Values.integrations.itxIntegration.logsPVC.useDynamicProvisioning. Value can be either false or true." -}}
{{- end -}}

{{- $isValid := .Values.integrations.itxIntegration.logsPVC.accessMode -}}
{{- if not ( or (eq $isValid "ReadWriteOnce") (eq $isValid "ReadOnlyMany") (eq $isValid "ReadWriteMany")) -}}
{{- fail "Please specify .Values.integrations.itxIntegration.logsPVC.accessMode as one of these supported values - ReadWriteOnce | ReadOnlyMany | ReadWriteMany" -}}
{{- end -}}

{{- end -}}

{{- $isEnabled := .Values.integrations.itxaIntegration.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxaIntegration.enabled. Value can be either false or true." -}}
{{- end -}}

{{- if (eq $isEnabled "true") -}}

{{- $isEnabled := .Values.integrations.itxaIntegration.dataSetup.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxaIntegration.dataSetup.enabled. Value can be either false or true." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.image.repository -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.image.repository cannot be empty." -}}
{{- end -}}

{{- if not .Values.integrations.itxaIntegration.image.digest -}}
{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.image.tag -}}
{{- if eq $result "false" -}}
{{- fail "Please specify either .Values.integrations.itxaIntegration.image.tag or .Values.integrations.itxaIntegration.image.digest " -}}
{{- end -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.image.pullPolicy -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.image.pullPolicy cannot be empty." -}}
{{- else -}}
{{- $result := .Values.integrations.itxaIntegration.image.pullPolicy -}}
{{- if not (or (eq $result "IfNotPresent") (eq $result "Always") (eq $result "Never")) -}}
{{- fail ".Values.integrations.itxaIntegration.image.pullPolicy is not valid. Valid values are IfNotPresent , Always or Never" -}}
{{- end -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.appSecret -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.appSecret cannot be empty." -}}
{{- end -}}

{{- $isEnabled := .Values.integrations.itxaIntegration.secureDBConnection.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxaIntegration.secureDBConnection.enabled. Value can be either false or true." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.persistence.claims.name -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.persistence.claims.name cannot be empty." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.sso.host -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.sso.host cannot be empty." -}}
{{- end -}}

{{- $result := include "mandatoryArgumentsCheck" .Values.integrations.itxaIntegration.sso.port -}}
{{- if eq $result "false" -}}
{{- fail ".Values.integrations.itxaIntegration.sso.port cannot be empty." -}}
{{- else -}}
{{- $result := include "isPortValid" .Values.integrations.itxaIntegration.sso.port -}}
{{- if eq $result "false" -}}
{{- fail "Values.integrations.itxaIntegration.sso.port field is not valid." -}}
{{- end -}}
{{- end -}}

{{- $isEnabled := .Values.integrations.itxaIntegration.sso.ssl.enabled | toString -}}
{{- if not ( or (eq $isEnabled "false") (eq $isEnabled "true")) -}}
{{- fail "Please provide valid value for Values.integrations.itxaIntegration.sso.ssl.enabled. Value can be either false or true." -}}
{{- end -}}
